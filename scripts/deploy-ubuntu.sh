#!/usr/bin/env bash
# =============================================================================
# Ubuntu fresh install / deploy helper (Navidrome + NaviGlassPlayer)
# =============================================================================
# Typical Ubuntu fresh-install flow:
#
#   ssh your-user@your-server
#   git clone https://github.com/BASF2HD/NaviGlassPlayer.git ~/NaviGlassPlayer
#   cd ~/NaviGlassPlayer
#   bash scripts/deploy-ubuntu.sh --auto
#
...
#   - Opens the configured app ports in ufw
# =============================================================================
...
ENV_FILE="${DEPLOY_ENV_FILE:-$REPO_ROOT/deploy-ubuntu.env}"
...
prepare_git_settings() {
  ...
  command -v ssh >/dev/null 2>&1 || die "Missing ssh client. Install openssh-client."
}
...
if [[ "$CLONE_MODE" == true ]]; then
  ...
  if [[ "$ENV_FILE_EXPLICIT" == false ]]; then
    ENV_FILE="$DEST/deploy-ubuntu.env"
  fi
fi
...
print_env_template() {
  ...
  cat <<EOF
# Ubuntu deploy settings for scripts/deploy-ubuntu.sh
# Save this as deploy-ubuntu.env on the server. Do not commit it.
...
if [[ "$AUTO_DEPLOY" == true && ! -f "$ENV_FILE" ]]; then
  ...
  {
    echo "# AUTO-GENERATED $(date -u +%Y-%m-%dT%H:%M:%SZ) by deploy-ubuntu.sh --auto"
    print_env_template
  } >"$ENV_FILE"
fi
...
install_packages() {
  ...
  run apt-get update
  run apt-get install -y ca-certificates curl ffmpeg git gnupg openssh-client tar ufw
  ensure_nodejs
}
...
configure_firewall() {
  if [[ "$SKIP_INSTALL" == true || "$SKIP_FIREWALL" == true ]]; then
    echo "[6/7] Skipping ufw setup"
    return 0
  fi

  echo "[6/7] Opening app port $FIREWALL_PORT/tcp in ufw..."
  run ufw allow OpenSSH
  run ufw allow "${FIREWALL_PORT}/tcp"

  if [[ "$INSTALL_NAVIDROME" == "true" ]]; then
    echo "[6/7] Opening Navidrome port $NAVIDROME_PORT/tcp in ufw..."
    run ufw allow "${NAVIDROME_PORT}/tcp"
  fi
